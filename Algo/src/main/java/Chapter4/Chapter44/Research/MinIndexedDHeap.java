...
public class MinIndexedDHeap<T extends Comparable<T>> {
    ...
    // The Inverse Map (im) stores the indexes of the keys in the range [0, sz) which make up the priority queue. It should be noted that 'im' and 'pm' are inverses of each other, so: pm[im[i]] = im[pm[i]] = i
    public final int[] im;
